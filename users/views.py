...
def get_toped_up_inv_with_sponsors(request):
    data_col = ['Investor_Name', 'Investor_Email',  'Invested_Amount', 'Top_Up_Date' ,'Top_Up_Amount', 'New_Investment_Capital', 'Investment_Type',  'Investment_Started_Date']
    data = {
            'Investor_Name':[],'Investor_Email':[], 'Invested_Amount':[], 'Top_Up_Date':[] , 'Top_Up_Amount':[], 'New_Investment_Capital':[],  'Investment_Type':[], 'Investment_Started_Date':[],
                        }
    template = 'users/multiple_sponsor_check.html'
    start_date_str = '01/02/21 00:00:00'
    ...
    new_df.to_excel(f"top_ups_{start_date}_to_{end_date}.xlsx")
...
def get_specific_investment_type_by_date(request):
    data_col = ['Investor_Name', 'Investor_Phone', 'Investor_Address', 'Investor_Email',  'Invested_Amount', 'Investment_Duration', 'Investment_Started_Date', 'Investment_Maturity_Date', 'Investment_Plan_Type', 'Investment_Plan']
    data = {
            'Investor_Name':[],'Investor_Phone':[],  'Investor_Address':[], 'Investor_Email':[], 'Invested_Amount':[], 'Investment_Duration':[], 'Investment_Started_Date':[], 'Investment_Maturity_Date':[],  'Investment_Plan_Type':[], 'Investment_Plan':[]
                        }
    template = 'users/multiple_sponsor_check.html'
    start_date_str = '01/01/21 00:00:00'
    end_date_str = '07/06/21 00:00:00'
    start_date = datetime.strptime(start_date_str, '%d/%m/%y %H:%M:%S').date()
    # print(start_date)
    end_date = datetime.strptime(end_date_str, '%d/%m/%y %H:%M:%S').date()
    # print(end_date)
    cip_investments = UserInvestment.objects.filter(active=False, plan__category__name="CIP", completed=True).all()
    for inv in cip_investments:
        user_profile = inv.user
        # if inv.maturity_date.date() == start_date:
        ...
        data['Investor_Email'].append(f"{user_profile.user.email}")
        data['Invested_Amount'].append(inv.amount)
        data['Investor_Phone'].append(f"{user_profile.phone}")
        data['Investment_Duration'].append(f"{int(int(inv.maturity_days)/30)}")
        data['Investment_Started_Date'].append(f"{inv.created_at.date()}")
        data['Investment_Maturity_Date'].append(f"{inv.maturity_date.date()}")
        data['Investment_Plan_Type'].append(f"{inv.plan.category.name}")
        data['Investment_Plan'].append(f"{inv.plan.name}")
    ...
    new_df.to_excel(f"all_hip_investment_1.xlsx")
...
def get_total_uninvested_wallet(request):
    ...
    old_cip = UserInvestment.objects.filter(plan__category__name="CIP", active=True).all()
    print(old_cip)
    for inv in old_cip:
        if inv.can_top_up == False:
...
def get_capital_paybacks(request):
    

    data_col = ['Investor_Name', 'Investor_Email', 'Investor_Phone',  'Invested_Amount', 'Investment_Duration', 'Investment_Ended_Date', 'Investment_Plan_Type', 'Payback_Generated_Date']
    data = {
            'Investor_Name':[],  'Investor_Email':[], 'Investor_Phone':[], 'Invested_Amount':[], 'Investment_Duration':[], 'Investment_Ended_Date':[], 'Investment_Plan_Type':[], 'Payback_Generated_Date':[]
                        }
    template = 'users/multiple_sponsor_check.html'

    set_date_str = '22/04/21 00:00:00'
    set_date = datetime.strptime(set_date_str, '%d/%m/%y %H:%M:%S').date()

    all_paybacks = CapitalPaybacks.objects.all()
    for pay in all_paybacks:
        if pay.created_at.date() <= set_date:
            print(f"record available for {pay.user.user.email}")
            data['Investor_Name'].append(f"{pay.user.first_name} {pay.user.last_name}")
            data['Investor_Phone'].append(f"{pay.user.phone}")
            data['Investor_Email'].append(f"{pay.user.user.email}")
            data['Investment_Duration'].append(f"{int(int((pay.investment.maturity_days))/30)}")
            data['Invested_Amount'].append(f"{int(pay.investment.amount)}")
            data['Investment_Ended_Date'].append(f"{pay.investment.maturity_date}")
            data['Investment_Plan_Type'].append(f"{pay.investment.plan.category.name}")
            data['Payback_Generated_Date'].append(f"{pay.created_at.date()}")

    print(data)
    new_df = pd.DataFrame({key:pd.Series(value) for key, value in data.items() }, columns=data_col)
    new_df = new_df.drop_duplicates(keep = False, inplace = False)
    
    new_df.to_excel(f"all_capital_paybacks.xlsx")
...
# def process_cip_capital_refund(request):
#     #fetch all active cip investments 
#     all_active_cip = UserInvestment.objects.filter(active=True, completed=False, plan__category__name="CIP")
#     if all_active_cip.exists():
#         for investment in all_active_cip:
#             user_profile = investment.user
    
#             print(f"{user_profile} - {investment.amount} - {investment.txn_code}")
#             #investemt operations
#             investment.completed = True
#             investment.active = False
#             investment.has_been_rolled_over = True 
#             investment.profit_earned = 0
#             investment.profit_paid = 0 
            
#             investment.save()


#             #fetch user investment earning
#             user_earning = UserInvestmentEarnings.objects.filter(plan=investment, user = user_profile).first()
#             user_earning.amount = 0.00
#             user_earning.active = False 
#             user_earning.completed = True
#             user_earning.save()


#             #generate transaction 
#             transaction_code = f"RFD-{str(''.join(random.choices(string.ascii_uppercase + string.digits, k = 7)))}-{user_profile.user_code}"
#             new_roi_tnx = Transaction.objects.create(amount=investment.amount, txn_code=transaction_code, user = user_profile, txn_method='manual' ,txn_type='withdrawal')

#             #generate capital payback
#             new_capital_payback = CapitalPaybacks.objects.create(user=user_profile, investment=investment)
#             new_capital_payback_notification =  UserNotifications.objects.create(user=user_profile, message=f"A capital refund of {int(investment.amount)} has been initiated for investment {investment.txn_code}")

#             #fetch user wallet 
#             user_wallet = MainWallet.objects.filter(user=user_profile).first()
#             user_wallet.deposit += int(investment.amount)
#             user_wallet.save()

#             # generate user notification 
#             new_wallet_credit_notification =  UserNotifications.objects.create(user=user_profile, message=f"Your wallet has been credited with a capital refund of {int(investment.amount)}")

#             try:
#                 # Email to User 
#                 subject, from_email, to = 'REFUND OF CIP INVESTMENT CAPITAL!', 'PIPMINDS ADMIN<hello@pipminds.com>', [ user_profile.user.email ]

#                 html_content = render_to_string(
#                     'events/cip_capital_refund.html', {
#                         'email': investment.user.user.email,
...
#                 print(f"email sent to {user_profile.user.email}")
...
#             print(f"done with {user_profile} ")
...
def get_all_partners(request):
    ...
    data_col = ['Partner_Name', 'Partner_Email', 'Partner_Phone','Number_Of_Direct_Leg' , 'Registration_Date', 'Has_Sponsor', 'Sponsor_Fullname', 'Sponsor_Email', 'Sponsor_Phone']
    data = {
            'Partner_Name':[],  'Partner_Email':[], 'Partner_Phone':[],'Number_Of_Direct_Leg':[], 'Registration_Date':[], 'Has_Sponsor':[], 'Sponsor_Fullname':[], 'Sponsor_Email':[], 'Sponsor_Phone':[]
                        }
    template = 'users/multiple_sponsor_check.html'

  

    all_partners = Profile.objects.filter(ppp_verfied=True).all()
    for prof in all_partners:
        data['Partner_Name'].append(f"{prof.first_name} {prof.last_name}")
        data['Partner_Email'].append(f"{prof.user.email}")
        data['Partner_Phone'].append(f"{prof.phone}")
        if prof.pioneer_ppp_member == True:
            print(f"piooner {prof.user.email}")
            reg_date = prof.pioneer_ppp_reg_expires - timedelta(days=365)
            data['Registration_Date'].append(f"{reg_date}")
        else:
            # check_sub = Subscription.objects.filter(user)
            print(prof.user.email)
            if prof.profile_subscription_instance.exists():
                reg_date = prof.profile_subscription_instance.first().created_at
                data['Registration_Date'].append(f"{reg_date.date()}")
            else:
                data['Registration_Date'].append(f"")

        check_downline = UserReferrals.objects.filter(downline=prof)
        if check_downline.exists():
            the_downline = check_downline.first()
            sponsor = the_downline.sponsor

            data['Has_Sponsor'].append("YES")
            data['Sponsor_Fullname'].append(f"{sponsor.first_name} {sponsor.user.last_name}")
            data['Sponsor_Email'].append(f"{sponsor.user.email}")
            data['Sponsor_Phone'].append(f"{sponsor.phone}")
        else:
            data['Has_Sponsor'].append("NO")
            data['Sponsor_Fullname'].append(" ")
            data['Sponsor_Email'].append(" ")
            data['Sponsor_Phone'].append(" ")
            
        check_ref_rec = UserReferrals.objects.filter(sponsor=prof)
        if check_ref_rec.exists():
            all_downlines = check_ref_rec.count()
            data['Number_Of_Direct_Leg'].append(all_downlines)
        else:
            data['Number_Of_Direct_Leg'].append(0)
    ...
    new_df.to_excel(f"all_partners.xlsx")
...
class CheckUserSponsor(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            data_col = ['Investor_Full_Name','Investor_Phone', 'Investor_Email',  'Amount', 'Duration', 'Investment_Date', 'Has_Sponsor', 'Sponsor_Fullname', 'Sponsor_Email', 'Sponsor_Phone' ]
            data = {
                  'Investor_Full_Name':[], 'Investor_Phone':[], 'Investor_Email':[],  'Amount':[], 'Duration':[], 'Investment_Date':[], 'Has_Sponsor':[], 'Sponsor_Fullname':[], 'Sponsor_Email':[], 'Sponsor_Phone':[]  
                                }
            if form.is_valid():
                ...
                for index, row in the_import.iterrows():
                    ...
                    data['Investor_Email'].append(row['email'])
                    data['Amount'].append(row['amount'])
                    data['Investment_Date'].append(row['investment date'])
                    data['Investor_Phone'].append(row['phone'])
                    data['Investor_Full_Name'].append(row['name'])
            ...
            new_df = pd.DataFrame({key:pd.Series(value) for key, value in data.items() }, columns=data_col)
            new_df = new_df.drop_duplicates(keep = False, inplace = False)
            new_df.to_excel(f"all_hip_part1.xlsx")
            return HttpResponseRedirect(reverse("users:custom_query"))
...
class ValidateEmail(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            data_col = ['Email','Response_Message']
            data = {
            ...
            if form.is_valid():
                ...
                for index, row in the_import.iterrows():
                    ...
                    data['Email'].append(the_email)


                   
                    match = re.match('^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$', the_email)

                    if match == None:
                        data['Response_Message'].append("Bad Syntax")
                        print('Bad Syntax')
                        # raise ValueError('Bad Syntax')
                    else: 
                        
                        feed_data = {"email": the_email}
                        ...
                        response = requests.post("https://verifyemailaddress.com/result", headers=headers, data=feed_data).text

                        if "is valid" in response:
                            data['Response_Message'].append("Email Exists")
                            ...
                            if "Email account exists." in response:
                                data['Response_Message'].append("Email Exists")
                                print(f"Email exist after second check")
                            else:
                                data['Response_Message'].append("Email does not Exists")
            ...
            new_df = pd.DataFrame({key:pd.Series(value) for key, value in data.items() }, columns=data_col)
            new_df = new_df.drop_duplicates(keep = False, inplace = False)
            new_df.to_excel(f"email_verifier.xlsx")
            return HttpResponseRedirect(reverse("users:custom_query"))
